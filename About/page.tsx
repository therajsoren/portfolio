const AboutMe = () => {
  return (
    <div id="about">
      <div className="w-full h-full lg:text-left p-4">
        <h1 className="text-4xl -tracking-wider font-black pb-14">About Me</h1>
        <p className="text-base pt-4 leading-normal tracking-tighter  dark:text-slate-200 text-slate-950 font-extralight">
          Hey, I'm Raj Soren, a passionate Full Stack Developer.
          <span className="inline lg:block">
            BIT Sindri '26. Building with Next.js, React & Tailwind CSS.
            Passionate about clean code and great UX.
          </span>
        </p>
      </div>
    </div>
  );
};
export default AboutMe;
